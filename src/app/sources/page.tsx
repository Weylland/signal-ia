...
function health(s: Source): { cls: string; color: string; label: string; labelEn: string } {
  if (s.lastStatus === "error" || s.lastError) return { cls: "s-err", color: "var(--er)", label: "Erreur", labelEn: "Error" };
  if (!s.lastFetchAt) return { cls: "s-wn", color: "var(--wn)", label: "En attente", labelEn: "Pending" };
      ...
      <PageBand>
        <div className="mag mb-12">
          {sources.map((s) => {
            ...
            const h = health(s);
            const last = s.lastFetchAt
              ? new Date(s.lastFetchAt).toLocaleDateString(locale, { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })
              : "—";
            return (
              <div
                key={s.id}
